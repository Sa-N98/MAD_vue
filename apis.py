...
class playList(Resource):
    def post(self):
        ...
        if playlist.searchQuery.filter(playlist.playlistName ==  playlist_name).first():
        ...
        pl= playlist.searchQuery.filter(playlist.playlistName ==  playlist_name).first()
...
class api_songs(Resource):
    def get(self, tag, searchQuery):
        # parser = reqparse.RequestParser()
        # parser.add_argument('tag', required=True, help="Playlist name is required")
        # parser.add_argument('searchsearchQuery')

        print(tag, searchQuery )


        if tag == 'Song':
                data = songs.query.filter(songs.name.like('%'+searchQuery+'%')).all()
                ...
                return {"api_data": song_data}
        elif tag == 'Artist':
                data = songs.query.filter(songs.artist.like('%'+searchQuery+'%')).all()
                ...
                data = songs.query.filter(songs.album.like('%'+searchQuery+'%')).all()
                song_data={}
